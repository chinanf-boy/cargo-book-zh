...
`cargo run` [*options*][`--` _args_]

## DESCRIPTION

运行二进制文件或本地包的示例。

两个破折号后面的所有参数(`--`)传递给要运行的二进制文件。如果您同时向Cargo和二进制文件传递参数，则后面的参数`--`进入二进制，在进入Cargo之前。
...
当没有给出目标选择选项时，`cargo run`将运行二进制目标。如果有多个二进制目标，则必须传递目标标志才能选择一个。或者`default-run`字段可以在`[package]`部分`Cargo.toml`选择默认运行的二进制文件的名称。
...
1.  构建本地包并运行其主要目标（假设只有一个二进制）：

    Cargo运输

2.  运行带有额外参数的示例：

    Cargo运行--示例exname--exoption exarg1 exarg2
